...
public class UnMapper {
    ...
    public static int[][] unMap(int[] solution) {
        ...
        for (int i = 111; i < solution.length; i++) {
            ...
            //variable is true.  This corresponds to a square on the puzzle:
            ...
            if (solution[i] > 0 && i < 1000) {
                ...
                if (value != 0 && column != 0) {
                    ...
                    // TODO if this branch is reachable, why? Doesn't it indicate bad data? -- SB
                }
            } else {
                // TODO if this is reachable, then what? It means the solver created bad data. If it's not reachable, why are we doing the test? -- SB
            }
        }//end for
        ...
        for (int i = 1; i <= 9; i++) {
            for (int j = 1; j <= 9; j++) {
                output.append(sudokuPuzzle[translate(i)][translate(j)]).append(" ");
            }
            output.append("\n");
        }

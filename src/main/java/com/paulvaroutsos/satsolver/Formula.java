package com.paulvaroutsos.satsolver;
...
public class Formula {
	...
	public Stack<LinkedList<Integer>> currentFormula = new Stack<>();
	...
	private TreeSet<Integer> unitVariables;
    ...
    public Formula(StringBuilder formulaString) {
        ...
        while (sc.hasNextLine()) {
            ...
            if (temp.startsWith("c")) {
                comment = (comment == null ? new StringBuilder() : comment).append(temp).append("\n");
            ...
            // Put the value into the formula
                ...
                if (clauseCount != clauseTotal) {
                    ...
                    originalFormula.add(Integer.valueOf(clauseCount));
        ...
        variablesAffected.add(new LinkedList<>());
    ...
    private void unitProp() {
        ...
        while (!unitVariables.isEmpty()) {
            ...
            for (Integer curClause : newFormula) {
                ...
                for (int j = 0; j < formula[curClause.intValue()].length; j++) {

                    int variableToCheck = formula[curClause.intValue()][j];
                ...
                if (!clauseRemoved) {
                    ...
                    if (trueClauseSize == 1) {
                        for (int i = 0; i < formula[curClause.intValue()].length; i++) {

                            if (currentAssignment[Math.abs(formula[curClause.intValue()][i])] == UNASSIGNED) {
                                ...
                                if (unitVariables.contains(Integer.valueOf(-formula[curClause.intValue()][i]))) {
                                    hasEmptyClause = true;
                                    currentFormula.push(newFormula);
                                    variablesAffected.push(variablesSetbyUP);
                                    return;
                                } else {
                                    unitVariables.add(Integer.valueOf(formula[curClause.intValue()][i]));
                                    break;
                                }
                            }
                        }//end for
    ...
    private void computeRanks() {
        resetRanks();
        double tempNewHighestRankValue;
        ...
        for (Integer clause : currentFormula.peek()) {
            ...
            for (int i = 0; i < formula[clause.intValue()].length; i++) {
                int varToCheck = formula[clause.intValue()][i];
                if (currentAssignment[Math.abs(varToCheck)] == UNASSIGNED) {
                    trueClauseSize++;
                }
            }
            ...
            for (int i = 0; i < formula[clause.intValue()].length; i++) {
                ...
                int tempVar = formula[clause.intValue()][i];
     ...
     *
     * @param var - the variable we want to propagate on,
     */
    public void setVariable(int var) {
        if (unitVariables.isEmpty()) {
            unitVariables.add(Integer.valueOf(var));
        }
    ...
    public void undoProp() {
        ...
        if (variablesAffected.size() > 1) {
            ...
            for (Integer i : temp) {
                currentAssignment[Math.abs(i.intValue())] = UNASSIGNED;
            }
        }
    ...
    public boolean isEmpty() {
        return currentFormula.peek().isEmpty();
    }
    ...
    public String toString() {
        ...
        for (int i = 1; i < currentAssignment.length; i++) {
            temp.append("Variable ").append(i).append(" Value ").append(currentAssignment[i]).append("\n");
        }

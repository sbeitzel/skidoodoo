...
public class SudokuGenerator {
			...
			for (int i = 0; i < Sudoku.MAX_VALID_SUDOKU_NUMBER; i++) {
				...
				if (row > 0) {
					...
					int _columnUpperBandToBeCopied = (c + row)
							% Sudoku.MAX_VALID_SUDOKU_NUMBER;
					k = matrix[_rowUpperBandToBeCopied][_columnUpperBandToBeCopied].intValue();

				}
				matrix[r][c] = Integer.valueOf(k);
	...
	public static Sudoku generatePlayableRandomSudoku(Sudoku.SudokuLevel level) {
		...
		for (int i = 0; i < NUM_CELLS; i++) {
			n[i] = Integer.valueOf(i);
		}
		_shuffleArray(n);
		for (int i = 0; i < NUM_CELLS-level.getValorizedCells(); i++) {
			int row_cols = n[i].intValue();
			int r = row_cols/9;
			int c = row_cols%9;
			s.getMatrix()[r][c] = Integer.valueOf(0);
		}
		return s;
